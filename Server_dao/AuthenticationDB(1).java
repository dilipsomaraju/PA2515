...
public class AuthenticationDB {
	...
	private static String GET_USER_NICKNAME_SQL = "select nickName from userInfo where userId=?";
	...
	public static ContactList getContactList(String userId){
		...
		try {
			...
			while(rs.next()){
				...
				user.setNickName(getUserNickName(user.getUserId()));
				users.add(user);
			}
	...
	public static String getUserNickName(String userId) {
		Connection conn = DB.getConn();
		PreparedStatement stmt = DB.getStmt(conn, GET_USER_NICKNAME_SQL);
		ResultSet rs = null;
		try {
			stmt.setString(1, userId);
			rs = DB.getRs(stmt);
